...
/**
 * MyContract v4.1 — Enterprise & Audit-Grade Solidity Contract
 *
 * Author: Kelvin
 *
 ...
 *
 * Changelog vs v4.0:
 * - Fixed name collision between `error ContractPaused()` and
 *   `event ContractPaused(...)` (this would not compile before).
 * - Fixed potential underflow in getHistory() when offset > history.length.
 * - Fixed totalAdminsAdded not being decremented on removeAdmin().
 * - Added two-step ownership transfer (transferOwnership + acceptOwnership)
 *   so ownership can no longer be lost to a typo'd address.
 * - Added withdraw() so ETH sent to the contract isn't stuck forever.
 * - Added cancelAdminProposal() and a couple of extra view helpers.
 * - Replaced invalid "GNU" SPDX identifier with "MIT".
 */
contract MyContract {
    // Versioning & Metadata
    string public constant CONTRACT_NAME = "MyContract";
    string public constant CONTRACT_VERSION = "4.1.0";
    ...
    event OwnershipTransferStarted(address indexed oldOwner, address indexed proposedOwner);
    ...
    event AdminProposalCancelled(uint256 indexed id);
    ...
    function getHistory(uint256 offset, uint256 limit) external view returns (HistoryEntry[] memory) {
        if (offset > history.length) revert InvalidOffset();
    ...
    function getLatestHistoryEntry() external view returns (HistoryEntry memory) {
        require(history.length > 0, "No history yet");
        return history[history.length - 1];
    }

    function getAdminProposal(uint256 id) external view returns (AdminProposal memory) {
        require(id < adminProposals.length, "Invalid proposal ID");
        return adminProposals[id];
    }

    function getAdminProposalsLength() external view returns (uint256) {
        return adminProposals.length;
    }
    ...
    function cancelAdminProposal(uint256 id) external onlyOwner {
        require(id < adminProposals.length, "Invalid proposal ID");
        AdminProposal storage proposal = adminProposals[id];
        require(!proposal.executed, "Already executed");
        ...
        emit AdminProposalCancelled(id);
    }
    ...
    function removeAdmin(address admin) external onlyOwner {
        ...
        admins[admin] = false;
        if (totalAdminsAdded > 0) {
            totalAdminsAdded--;
        }
        emit AdminRemoved(admin);
    }

    // Ownership (two-step, safer than a single-shot transfer)
